...
def start_end(start, end):
    ...
    date_range_max = session.query(Measurement.date).order_by(Measurement.date.desc()).first()[0]
    date_range_min = session.query(Measurement.date).first()[0]
    valid_entry_start = session.query(exists().where(Measurement.date.between(start.split('/')[0], start.split('/')[1]))).scalar()
    valid_entry_end = session.query(exists().where(Measurement.date.between(end.split('/')[0], end.split('/')[1]))).scalar()
